...
**Non ancora verificato manualmente nel browser** (solo `pnpm typecheck`/`lint`/`test`/`build` — quest'ultimo di `apps/web` non esegue `tsc` sui file `.svelte`, quindi non garantisce la correttezza dei tipi tra i prop dei componenti). Prossimo passo naturale: un giro manuale end-to-end nel browser (crea sfida, join da un secondo utente, gioca un match, verifica il settle).
